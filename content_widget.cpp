...
ContentWidget::ContentWidget(int room_id, TreeModel *model, QWidget *parent)
{
    ...
    if (0 == i_scene_num)
    {
		...
		no_scene->setGeometry(QRect(0, 80, 320, 60*4)); //4倍行距
		no_scene->setWordWrap(true);
		no_scene->setAlignment(Qt::AlignCenter);
		return;
    }
	...
	for(i=0; i<i_scene_num; i++)
	{
		...
		if (i_scene_num == 1)
		{
		    tool_button->setGeometry(QRect(95+320, 95+2, SCENE_BUTTON_X+30, SCENE_BUTTON_Y+45));//130
...
void ContentWidget::mouseReleaseEvent(QMouseEvent *event)
{
   // printf("ContentWidget::mouseReleaseEvent\n");
    ...
    if (do_scene_flag)
    {
        //qDebug()<<"line="<<__LINE__<<", do_scene_flag";
        return;
    }
