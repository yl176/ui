...
{ 
	...
	dev_label = new QLabel(this);
	dev_label->setText("ON");
	dev_label->setStyleSheet("background-color:rgb(100, 100, 100);font-size:35px;color:white;)"\
					"border-radius:25px;font-weight:normal;");
	dev_label->setFixedSize(60, 60);
	dev_label->setGeometry(QRect(260, 0, 60, 60));
	dev_label->setVisible(1);
	m_nLabelSize = 50;
		// 连接信号槽 
...
{ 
	...
	path.addRoundedRect(QRectF(m_nMargin, m_nMargin, width() - 2 * m_nMargin - m_nLabelSize, height() - 2 * m_nMargin), m_radius, m_radius);
...
{ 
	...
	{ 
		m_nX += 1; 
		if (m_nX + m_nLabelSize>= width() - m_nHeight)
		{
			m_timer.stop(); 
			dev_label->setText("OFF");
			dev_label->setGeometry(QRect(0, 0, 60, 60));
		}
	} 
	else 
	{ 
...
{ 
    //qDebug()<<"setToggle checked="<<checked;
	m_bChecked = checked; 
	m_timer.start(10); 
} 
